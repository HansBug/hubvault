...
modnames = [mname for mname in sys.modules if mname.startswith('hubvault')]
...
if not os.environ.get("NO_CONTENTS_BUILD"):
    ...
    if _ON_READTHEDOCS:
        print("Read the Docs environment detected, skipping project-level build bootstrap.")
    else:
...
from hubvault.config.meta import __TITLE__, __AUTHOR__, __VERSION__
