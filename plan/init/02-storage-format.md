...
为了兼容 Python 3.7-3.14，并保持公开哈希语义与 HF/Git 风格模型稳定，格式 v1 当前仍采用：
...
- 即便 Phase 10 已在 chunk 规划层直接引入 `blake3` 与 `fastcdc`，公开对象 ID、公开文件 `sha256`、Git 风格 `oid` 仍继续保持这套稳定语义，不把内部优化泄露到兼容层

### 3.2 当前内部优化与后续扩展

当前已经落地：

- `blake3` 作为 FastCDC chunk 规划阶段的快速哈希函数
- `fastcdc` 作为默认内容定义分块算法

后续仍可继续评估：

- `zstd` 压缩
- 更激进的 metadata / IO 热点优化
