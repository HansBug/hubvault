...
* [ ] 用 Phase 12 扩容后的 benchmark 结果重新排序热点，优先锁定 `read_range()`、cold/warm `hf_hub_download()` 与 `snapshot_download()` 的读路径回退。
...
* [ ] 把 `read_bytes()`、`read_range()`、`hf_hub_download()` 从单文件读先 `_snapshot_for_revision()` 的路径上拆下来，新增 direct path resolver。
* [ ] 为 `_read_chunked_file_range()` 增加调用级 manifest cache。
* [ ] 为 `_read_chunked_file_range()` 增加调用级 index segment cache。
* [ ] 为 `_read_chunked_file_range()` 增加 batched chunk resolve，避免 per-chunk 重复 `lookup()`。
* [ ] 为 `_read_chunked_file_range()` 增加 pack reader / file descriptor reuse。
* [ ] 为 `_read_chunked_file_range()` 引入更低开销的输出 buffer 组织，减少小片段 `bytes` 拼接。
* [ ] 为 warm `hf_hub_download()` 增加 managed-view metadata fast path，命中时直接返回现有 detached view。
* [ ] 为 `_materialize_content_pool()` 增加 unchanged no-op fast path，避免重复落盘与 `fsync`。
* [ ] 为 `_ensure_detached_view()` 增加 sidecar/metadata-first fast path，避免 warm path 下对现有目标做全文件 `read_bytes() + sha256`。
* [ ] 为 `snapshot_download()` 增加 `commit_id + allow_patterns + ignore_patterns` 级 quick return。
* [ ] 为 `snapshot_download()` 增加 selective repair，只重建缺失或失配文件。
...
* [ ] 每轮优化后都记录相对 host local sequential I/O reference 的比值变化，避免只看绝对吞吐数值。
...
* [ ] `hf_hub_download_warm` 在 `standard` 档达到 `>= 900 MiB/s`。
* [ ] `hf_hub_download_cold` 在 `standard` 档达到 `>= 600 MiB/s`。
* [ ] `cache_heavy_warm_download` 在 `standard` 档达到 `>= 900 MiB/s`。
* [ ] `history_deep_listing` 的 `wall_clock_seconds` 降到 `<= 3.2s`。
* [ ] `read_range()` 至少获得 `2x` 级别的稳定收益，且没有把写路径和空间放大重新打坏。
* [ ] warm `hf_hub_download()` 至少获得 `2x` 级别的稳定收益，且没有把缓存语义和 detached view 语义重新打坏。
* [ ] cold `hf_hub_download()` 至少获得 `2x` 级别的稳定收益，且没有把缓存语义和 detached view 语义重新打坏。
* [ ] `snapshot_download()` 至少获得 `2x` 级别的稳定收益，且没有把路径保真和 selective repair 语义打坏。
