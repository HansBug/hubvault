...
- 当前 Phase 12 已完成 benchmark 扩容与指标口径固化：benchmark 结果现已覆盖 bandwidth / metadata / maintenance / amplification / stability 五类摘要，携带 machine signature、dataset shape、threshold policy、manifest、same-machine compare 产物，并额外记录 host local sequential I/O reference。
...
* [ ] 为 `read_range()` 固定 `cProfile` / `py-spy` / opt-in `tracemalloc` 的 profiling 命令、输入 shape 与产物保存位置。
* [ ] 为 warm `hf_hub_download()` 固定 `cProfile` / `py-spy` / opt-in `tracemalloc` 的 profiling 命令、输入 shape 与产物保存位置。
* [ ] 为 cold `hf_hub_download()` 固定 `cProfile` / `py-spy` / opt-in `tracemalloc` 的 profiling 命令、输入 shape 与产物保存位置。
* [ ] 为 `snapshot_download()` 固定 `cProfile` / `py-spy` / opt-in `tracemalloc` 的 profiling 命令、输入 shape 与产物保存位置。
* [ ] 为 `history_deep_listing` / `list_repo_commits()` / `list_repo_refs()` / `list_repo_reflog()` 固定 profiling 命令、输入 shape 与产物保存位置。
* [ ] 为 `merge_heavy_non_fast_forward` 固定 profiling 命令、输入 shape 与产物保存位置。
...
* [ ] 为 `_ensure_detached_view()` 增加 sidecar/metadata-first fast path，避免 warm path 下对现有目标做全文件 `read_bytes() + sha256`。
...
* [ ] 在 13A / 13B 收益不足时，再评估 `pread` / `mmap` / verified chunk cache，并明确跨平台 fallback。
...
- benchmarked code commit: `9b5e14010608c31b271459262dcaf1c540b5d1ee`（`9b5e140`）
- compare baseline commit: `26a198711dc41e1bf2ec091361f4b64543a69210`（`26a1987`）
- completed regression / benchmark:
  `make unittest`
  `./venv/bin/python -m tools.benchmark.run_phase9 --scale standard --scenario-set full --output build/benchmark/phase13/summary/phase13-standard-full-9b5e140.json --manifest-output build/benchmark/phase13/manifests/phase13-standard-full-9b5e140-manifest.json`
  `./venv/bin/python -m tools.benchmark.compare build/benchmark/phase12/summary/phase12-standard-full.json build/benchmark/phase13/summary/phase13-standard-full-9b5e140.json`
- core artifacts:
...
- representative results vs Phase 12 baseline:

| scenario | Phase 12 baseline | Phase 13 (`9b5e140`) | delta vs baseline | note |
| --- | --- | --- | --- | --- |
| `large_upload` | `208.70 MiB/s`, `p50 0.138884s` | `198.18 MiB/s`, `60.91%` of host write baseline, `p50 0.144452s` | throughput `-5.04%`, p50 `+4.01%` | 写路径没有继续提升，但仍接近本机顺序写上限的一半以上 |
| `large_read_range` | `223.96 MiB/s`, `p50 0.126996s` | `204.54 MiB/s`, `3.15%` of host read baseline, `p50 0.129064s` | throughput `-8.67%`, p50 `+1.63%` | 仍是当前最主要的未解瓶颈 |
| `hf_hub_download_cold` | `281.80 MiB/s`, `p50 0.206108s` | `534.09 MiB/s`, `8.24%` of host read baseline, `p50 0.188860s` | throughput `+89.53%`, p50 `-8.37%` | 已明显逼近 `>= 600 MiB/s` hard target，但还差最后一段 |
| `hf_hub_download_warm` | `437.56 MiB/s`, `p50 0.232882s` | `15768.73 MiB/s`, `243.13%` of host read baseline, `p50 0.191718s` | throughput `+3503.81%`, p50 `-17.68%` | warm path 已基本收敛，不再是 Phase 13 主战场 |
| `cache_heavy_warm_download` | `450.89 MiB/s`, `p50 0.813272s` | `21828.10 MiB/s`, `336.56%` of host read baseline, `p50 0.562698s` | throughput `+4741.06%`, p50 `-30.81%` | cache-heavy warm 场景也已被解决 |
| `snapshot_download_small` | `1.06 MiB/s`, `p50 1.338577s` | `15.49 MiB/s`, `cache_amplification 1.053375`, `p50 0.927746s` | throughput `+1360.72%`, p50 `-30.69%` | quick return / selective repair 已带来稳定的 `> 2x` 级收益 |
| `mixed_model_snapshot` | `228.24 MiB/s`, `p50 0.527436s` | `465.62 MiB/s`, `cache_amplification 1.000066`, `p50 0.474930s` | throughput `+104.01%`, p50 `-9.95%` | mixed-model snapshot 也已完成 `> 2x` 级提升 |
| `history_deep_listing` | `8157.42 ops/s`, `wall 4.942012s` | `7204.14 ops/s`, `wall 4.662651s` | ops/s `-11.69%`, wall `-5.65%` | wall-clock 有改善，但距离 `<= 3.2s` 目标仍远，13B 尚未开始 |
| `merge_heavy_non_fast_forward` | `48.56 MiB/s`, `p50 0.441694s` | `82.61 MiB/s`, `p50 0.407254s` | throughput `+70.12%`, p50 `-7.80%` | 读路径优化对 merge-heavy 也有间接收益 |
| `verify_heavy_full_verify` | `43.52 MiB/s`, `p50 1.409059s` | `28.69 MiB/s`, `p50 1.705525s` | throughput `-34.07%`, p50 `+21.04%` | 当前唯一触发 compare alert 的回退项，需要单独 profiling |
- current judgment:
  warm `hf_hub_download()` 与 `cache_heavy_warm_download()` 已经从主要问题变成已收敛项。
  cold `hf_hub_download()` 与 `snapshot_download()` 已经获得显著收益，cache amplification 也已从约 `2x` 收敛到约 `1x`。
  `read_range()` 仍然是最主要的性能缺口，而且这轮 full benchmark 的绝对吞吐没有上升，说明 13A 已做完的“低风险清扫”还不够。
  metadata-heavy 路径只拿到了读侧优化的间接收益，真正的 object payload / public oid / reflog parse cache 还没有做。
  `verify_heavy_full_verify` 仍是当前最明显的残余风险，需要在 13B 前先补一轮单独画像。
