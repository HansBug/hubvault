...
- 当前剩余工作已经收敛为两个方向：
  ...
  Phase 14 则基于新的 reachable-state 定义重审 Windows / 断电耐久性边界、同进程/跨进程/跨节点可串行化边界，并规划 metadata 事务化重构。
...
* [ ] 把计划文档与后续测试口径统一到 reachable-state safety 定义。
* [ ] 复盘 `backend.py` 当前 failpoint 证据，明确 ref-changing 与 storage-only 路径的不同要求。
* [ ] 比较 `sqlite3`、`lmdb` 与继续维护当前文件协议三条路线，覆盖安全、性能、迁移、跨平台成本和跨节点共享 FS 可串行化成本。
* [ ] 固化同进程多线程、同机多进程、跨节点共享路径三类 serializability 测试矩阵。
* [ ] 设计 Phase 14 MVP 的 metadata schema、恢复状态机与迁移策略。
* [ ] 固化 zip 打包迁移回归，确保仓库在任意新路径解压后可直接启用。
* [ ] 明确 payload files 继续外置时，Windows 上还剩哪些 durability 证明缺口。

### Checklist

* [ ] 不再把孤儿对象、残留事务目录或 stale detached view 混同为数据损坏。
* [ ] 已明确 Phase 14 MVP 的默认 metadata substrate 与 Python 版本策略。
* [ ] 已明确新旧实现切换时的迁移和回滚边界。
* [ ] 已明确同进程、同机多进程和跨节点共享路径的全局线性化机制。
* [ ] 已明确并保持“zip 级可移植”的严格自包含语义。
* [ ] Phase 14 的后续性能项都建立在新的安全定义之上，而不是为了 benchmark 牺牲恢复语义。
