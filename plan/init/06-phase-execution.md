...
4. 再补极端场景与故障注入测试，把 reachable-state 安全与 rollback-only 边界压实
...
10. 再基于新的 reachable-state 定义重审 durability、Windows 边界、全局可串行化并发边界与 metadata 事务化重构
11. 最后在 Phase 14 的设计定稿基础上，执行 repo-root SQLite truth-store 的全面迁移，彻底收束剩余文件协议真相层
...
- 当前仓库并发与恢复基线已经收敛为 repo-root 内单一 `repo.lock` shared/exclusive 文件锁 + rollback-only 恢复：多个 reader 可并发，writer 独占；同进程线程、同机多进程和共享路径访问都走同一套文件锁协议；中断写事务只回滚，不继续补完。
...
- 当前剩余工作已经收敛为两个方向：
  Phase 13 继续收敛剩余时间路径热点与局部回退风险；
  Phase 15 则承接 Phase 14 已定稿的 SQLite-first 设计，执行 repo-root truth-store 的全面迁移。
- Phase 14 已经完成 reachable-state safety、serializability、zip 级可移植与 SQLite-first 选型/设计闭环，不再继续占用实现阶段。
...
15. reachable-state durability / Windows 边界 / 全局可串行化 / metadata 事务化重构
16. SQLite-first truth-store 全面迁移 / 恢复链路统一 / 文件协议真相层收束
...
系统化模拟极端场景、损坏中间态和中断写入，确保仓库可达真相永远不被半写状态污染；对 ref-changing 写路径，最坏结果只允许等效于“本次操作从未发生过”，对 storage-only 路径则允许留下后续可清理的不可达垃圾，但不允许 reachable truth 落入半提交状态。
...
- merge 路径：`merge()` 在 target ref/reflog 线性化边界上的中断；验证主分支 head、文件树和可达 reflog 语义都等效于本次操作从未发生过
...
- 当前 head、branch/tag 指向、tree/list/read 结果必须仍然等效于“本次写操作从未发生过”，或者已经完整成功；若留下垃圾，也只能是不可达且可后续清理的垃圾
...
## Phase 14. Reachable-State Durability 与 Metadata 事务化重构

### Goal

在新的 reachable-state 安全定义下，重新评估 `backend.py` 的断电/崩溃耐久性、Windows 语义边界、同进程/跨进程/跨节点可串行化边界、严格自包含/zip 级可移植约束、metadata fan-out 成本和候选依赖替换路线，明确下一轮应先做哪些安全硬化，再做哪些重构与性能收益回收。
...
详细定义、候选依赖、分批重构路线与 MVP cut 见 `plan/init/10-phase14-reachable-durability-and-metadata-redesign.md`。

### Technical Focus

Phase 14 的重点不是重新追求“零残留”，而是先把下面这些问题拆清楚：

- 什么才算真正的数据损坏，什么只是可 GC 的 residue。
- 当前 Windows / 断电语义下，哪些 durability 结论已经成立，哪些仍未被证明。
- 同进程线程、同机多进程和 NFS/SMB 等跨节点共享路径访问是否都能保证结果可串行化。
- metadata 重构后是否仍严格保持“zip 打包、搬走、解压、直接启用”的自包含约束。
- 当前自定义 `MANIFEST + JSONL segment + 多文件 metadata` 协议是否还值得继续维护。
- 是否应优先引入 repo-local transactional metadata substrate，例如 `sqlite3`，以及它能同时解决多少安全与性能问题。

### Todo

* [x] 把计划文档与测试口径统一到 reachable-state safety 定义。
* [x] 复盘 `backend.py` 当前 failpoint 证据，明确 ref-changing 与 storage-only 路径的不同要求。
* [x] 比较 `sqlite3`、`lmdb` 与继续维护当前文件协议三条路线，覆盖安全、性能、迁移、跨平台成本和跨节点共享 FS 可串行化成本。
* [x] 固化同进程多线程、同机多进程、跨节点共享路径三类 serializability 测试矩阵与当前公开回归基线。
* [x] 设计 Phase 14 MVP 的 metadata schema、恢复状态机与迁移策略。
* [x] 固化 zip 打包迁移回归，确保仓库在任意新路径解压后可直接启用。
* [x] 明确 payload files 继续外置时，Windows 上还剩哪些 durability 证明缺口。
* [x] 将全面 SQLite 实现移交 Phase 15，不再继续占用 Phase 14 scope。

### Checklist

* [x] 不再把孤儿对象、残留事务目录或 stale detached view 混同为数据损坏。
* [x] 已明确 Phase 14 MVP 的默认 metadata substrate 与 Python 版本策略。
* [x] 已明确新旧实现切换时的迁移和回滚边界。
* [x] 已明确同进程、同机多进程和跨节点共享路径的全局线性化机制。
* [x] 已明确并保持“zip 级可移植”的严格自包含语义。
* [x] Phase 14 的后续性能项都建立在新的安全定义之上，而不是为了 benchmark 牺牲恢复语义。
...
## Phase 15. 全面 SQLite 真相层迁移

### Goal

在 Phase 14 已经完成的 reachable-state 定义、并发边界与 SQLite-first 设计闭环基础上，执行 repo-root 内的全面 SQLite 改造，把剩余文件协议真相层收束到单一事务 substrate，同时保持 payload 文件外置、自包含/可搬迁、HF 兼容公开语义和 repo-level `repo.lock` 串行化边界不变。

### Status

已完成。

当前仓库已经完成 Phase 15 的 repo-root SQLite truth-store 迁移：`metadata.sqlite3` 承担 repo metadata、refs、reflog、txn log、visible chunk index 与 object metadata 的 steady-state truth-source，payload bytes 继续留在文件系统里，`repo.lock` 继续承担外层公开串行化边界。

基线 benchmark 证据固定在 commit `71420999d547f92a5f442f7bdea2dae6c4ca348d`，迁移后对比记录见 `build/benchmark/phase15/phase15-sqlite-benchmark-record.md`。按当前执行约束，最终实现状态保留为未提交工作树，因此 post-change 证据使用 `7142099-dirty` 标识。

### Detailed Plan

详细执行范围、分阶段切分、schema 目标、迁移策略与 MVP cut 见 `plan/init/11-phase15-full-sqlite-truth-store-migration.md`。

### Technical Focus

Phase 15 的重点不是把所有 payload bytes 塞进数据库，而是：

- 用标准库 `sqlite3` 建立 repo-root 内唯一的 metadata / object-truth store。
- 保留 `locks/repo.lock` 作为公开 serializability 边界，SQLite 只承担单仓 transaction 职责。
- 把 repo meta、refs、reflog、txn log、chunk visible、commit/tree/file/blob metadata 收进 SQLite。
- 保持 `objects/blobs/*.data` 与 `chunks/packs/*.pack` 继续留在文件系统。
- 用 SQLite transaction commit 取代当前多文件 publish 作为 metadata 线性化点。
- 在 Python 3.7-3.14、Windows/macOS/Linux、zip 级可移植和 shared-FS 保守边界下完成迁移。

### Todo

* [x] 新增 repo-local SQLite metadata/object truth-store，并保持 `FORMAT`、`repo.lock`、payload 文件与 detached view 的现有语义边界。
* [x] 先落地 `repo_meta`、`refs`、`reflog`、`txn_log`、`chunk_visible` 五类 Tier 1 真相层。
* [x] 再落地 `objects_commits`、`objects_trees`、`objects_files`、`objects_blobs` 四类 Tier 2 object metadata。
* [x] 将 `create_repo()`、`create_commit()`、`merge()`、ref 管理、`gc()`、`squash_history()` 的 metadata commit barrier 统一改到 SQLite transaction。
* [x] 将恢复流程改成“payload residue 清理 + SQLite metadata rollback / committed truth reopen”，不再依赖大量文件散布状态。
* [x] 为读路径补 SQLite adapter，收敛 `chunk visible` 与 object metadata 的全量 materialization 成本。
* [x] 在迁移过程中移除旧的 `repo.json` / `refs/` / `logs/refs/` / `chunks/index/MANIFEST` 真相职责，避免双真相源长期并存。
* [x] 为新的 SQLite 线性化点重写 failpoint、并发、zip/unzip、shared mount 验收与 benchmark 回归。

### Checklist

* [x] 新实现仍只依赖 repo root 内状态，不引入仓库外 sidecar 或外部数据库服务。
* [x] Python 3.7-3.14 与 Windows/macOS/Linux 兼容目标保持不变。
* [x] `sqlite3` 配置保持 `journal_mode=DELETE`，`synchronous` 优先 `EXTRA`、不支持时回退 `FULL`，默认禁用 WAL。
* [x] payload bytes 继续外置，`hf_hub_download()` / `snapshot_download()` 的路径保真与 detached view 语义不回退。
* [x] 公开 `oid` / `blob_id` / `sha256`、Git 风格 commit/tree OID、rollback-only 与 zip 级可移植语义不回退。
* [x] 小文件 commit、metadata-heavy listing、chunk lookup 与恢复路径相对当前文件协议有可量化收益。
* [x] 迁移失败时不会暴露双真相半状态；迁移完成后仓库在换路径、zip/unzip、换机器后仍可直接启用。
