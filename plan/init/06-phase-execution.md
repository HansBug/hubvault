...
* [ ] 新增 repo-local SQLite metadata/object truth-store，并保持 `FORMAT`、`repo.lock`、payload 文件与 detached view 的现有语义边界。
* [ ] 先落地 `repo_meta`、`refs`、`reflog`、`txn_log`、`chunk_visible` 五类 Tier 1 真相层。
* [ ] 再落地 `objects_commits`、`objects_trees`、`objects_files`、`objects_blobs` 四类 Tier 2 object metadata。
* [ ] 将 `create_repo()`、`create_commit()`、`merge()`、ref 管理、`gc()`、`squash_history()` 的 metadata commit barrier 统一改到 SQLite transaction。
* [ ] 将恢复流程改成“payload residue 清理 + SQLite metadata rollback / committed truth reopen”，不再依赖大量文件散布状态。
* [ ] 为读路径补 SQLite adapter，收敛 `chunk visible` 与 object metadata 的全量 materialization 成本。
* [ ] 在迁移过程中移除旧的 `repo.json` / `refs/` / `logs/refs/` / `chunks/index/MANIFEST` 真相职责，避免双真相源长期并存。
* [ ] 为新的 SQLite 线性化点重写 failpoint、并发、zip/unzip、shared mount 验收与 benchmark 回归。

### Checklist

* [ ] 新实现仍只依赖 repo root 内状态，不引入仓库外 sidecar 或外部数据库服务。
* [ ] Python 3.7-3.14 与 Windows/macOS/Linux 兼容目标保持不变。
* [ ] `sqlite3` 配置保持 `journal_mode=DELETE`，`synchronous` 优先 `EXTRA`、不支持时回退 `FULL`，默认禁用 WAL。
* [ ] payload bytes 继续外置，`hf_hub_download()` / `snapshot_download()` 的路径保真与 detached view 语义不回退。
* [ ] 公开 `oid` / `blob_id` / `sha256`、Git 风格 commit/tree OID、rollback-only 与 zip 级可移植语义不回退。
* [ ] 小文件 commit、metadata-heavy listing、chunk lookup 与恢复路径相对当前文件协议有可量化收益。
* [ ] 迁移失败时不会暴露双真相半状态；迁移完成后仓库在换路径、zip/unzip、换机器后仍可直接启用。
