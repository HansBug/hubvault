...
Phase 12 不会为了追求“像系统 benchmark 一样严苛”而在 CI 里尝试清空 page cache 或控制 CPU governor；它只要求同一 runner、同一配置、同一数据集下的结果可比较。
...
* [ ] 为 `nested-small`、`mixed-model`、`history-deep`、`merge-heavy`、`cache-heavy`、`verify-heavy` 实现确定性数据集生成器。
* [ ] 扩展 benchmark harness，让 metadata-heavy 场景输出 `operations_per_sec` 与 tail latency，而不是只有吞吐。
* [ ] 扩展 summary schema，加入 `latency_p50/p95/p99`、`latency_iqr`、`operations_per_sec`、`write_amplification`、`space_amplification_live`、`space_amplification_unique`、`cache_amplification` 等字段。
* [ ] 为运行结果补 machine signature 与 dataset shape 信息，并确保 compare 工具优先比较同机同配置结果。
* [ ] 调整 benchmark workflow，区分 `smoke`、`standard`、`nightly`、`pressure` 四档执行口径。
* [ ] 为 bandwidth / metadata / maintenance / amplification / stability 五类结果补独立摘要模板，不再输出单一总分。
* [ ] 明确哪些指标进入告警阈值，哪些指标只记录趋势。
* [ ] 把 Phase 12 扩容后的 benchmark 入口写回 Makefile、workflow 文档和计划说明。

## Checklist

* [ ] 所有新增指标都能明确说明分子、分母和适用场景。
* [ ] benchmark summary 不会再把 metadata-heavy 路径塞进 bulk throughput 一列里。
* [ ] amplification 指标已经正式进入结果 schema，而不只是人工解读时的辅助描述。
* [ ] benchmark 结果可以携带足够的环境元数据，支持长期比较和结果复核。
* [ ] Linux / Windows / macOS 三平台仍然保持公开 API / 公开 CLI 驱动的 benchmark 入口。
* [ ] Phase 12 的产物结构已经能支持后续 Phase 13 做热点排序和回归归因。
