...
- 文档中的删除线项表示这些实现/回归工作已整体移交给 Phase 15，不再继续占用 Phase 14 scope。
...
- CPython 标准库 `sqlite3` 满足当前仓库的 Python `3.7-3.14` 兼容目标；但 `synchronous=EXTRA` 不能在所有旧系统 SQLite 版本上无条件假定成立，Phase 15 落地时必须做 runtime capability probe，并在不支持时回退到 `FULL`。
...
| --- | --- | --- | --- | --- | --- | --- |
| metadata / refs / reflog / txn journal | 多文件 JSON + 文本 ref + reflog JSONL | `sqlite3`（stdlib） | 高 | stdlib、跨平台、支持 3.7-3.14、事务语义成熟、可把 metadata fan-out 收束到单一 durable commit | 默认不能用 WAL；跨节点共享路径依然依赖底层文件系统锁/rename/fsync 语义；旧系统 SQLite 上需要为 `EXTRA` 准备回退；需要 schema 与迁移重构 | Phase 14 默认方案：继续使用外层 `repo.lock`，内部采用 rollback journal（`DELETE`）+ `synchronous` 优先 `EXTRA`、不支持时回退 `FULL` |
...
- journal mode：`DELETE`
- synchronous：优先 `EXTRA`；若运行时 SQLite 不支持，则回退 `FULL`
- temp store：`MEMORY`
- WAL：默认禁用
...
- Phase 15 不能把 `EXTRA` 当成硬编码前提；为了保住 Python 3.7 / 旧系统平台面，PRAGMA 协商必须允许回退到 `FULL`。
...
新的 reachable-state 定义、并发/迁移矩阵和公开语义已经收口，并已有基础公开回归。更完整的 failpoint 套件和基于 SQLite 线性化点的并发/维护矩阵已移交 Phase 15。
...
* [x] 复核现有公开回归，确认 `after_reflog_append`、`after_ref_write`、`gc.after_publish`、同进程线程/同机多进程串行化与 zip/unzip 迁移已有基础信号。
...
* [x] 现有公开回归已经覆盖同进程线程、同机多进程与 zip/unzip reopen 的基础行为，不再把这部分基线误写成缺失。
...
* [x] 明确标准库 `sqlite3` 覆盖 Python `3.7-3.14`，并约定 `synchronous=EXTRA` 在不支持时必须回退到 `FULL`。
...
* [x] 已明确 SQLite 在旧系统上的 capability fallback，不把 `EXTRA` 当作无条件前提。
...
* [x] 锁定 SQLite-first 的 Tier 1 / Tier 2 范围、schema 草案、读写蓝图、默认 PRAGMA、迁移原则和执行顺序。
* [x] 锁定“repo.lock 外层串行化 + repo-root metadata.db + payload 文件继续外置”的主线，不再在 Phase 14 内继续摇摆依赖和协议。
...
* [x] Phase 14 已完成 SQLite-first 设计闭环，Phase 15 可以直接进入执行而无需重新选型。
...
原本希望在本阶段进一步直接落下、但现已整体移交 Phase 15 的内容为：
