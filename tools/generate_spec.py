#!/usr/bin/env python
"""Generate the maintained PyInstaller spec file for the standalone CLI."""
...
OPTIONAL_RUNTIME_PACKAGES: Sequence[str] = [
    ...
    'typing_extensions',
    'uvicorn',
    # Keep the SSL runtime explicit because it is only reached through the
    # optional server stack during CLI smoke tests.
    'ssl',
    '_ssl',
    '_hashlib',
]
...
def _analysis_optimize_line() -> str:
    """Return the Analysis optimize argument only when supported."""
    try:
        import PyInstaller
    except ImportError:
        return ''

    version_text = getattr(PyInstaller, '__version__', '')
    major_text = version_text.split('.', 1)[0]
    if major_text.isdigit() and int(major_text) >= 6:
        return '    optimize=0,\n'

    return ''


def _spec_platform_block() -> str:
    """Return small helper variables embedded into the generated spec."""
...
def generate_spec():
    ...
    platform_block = _spec_platform_block()
...
exe = EXE(
    ...
    # Windows PE binaries such as _ssl.pyd and libcrypto/libssl are sensitive
    # to strip/UPX in older Python runtimes. Keep Windows builds conservative
    # so packaged `hubvault serve` stays functional in Release Test.
    strip=not IS_WINDOWS,
    upx=not IS_WINDOWS,
    upx_exclude=['_ssl.pyd', '_hashlib.pyd', 'libcrypto-1_1.dll', 'libssl-1_1.dll'] if IS_WINDOWS else [],
    runtime_tmpdir=None,
    console=True,
